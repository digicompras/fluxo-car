...
$sql4 = "SELECT * FROM bco_entradas where num_fatura = '$num_fatura' and num_orcamento = '$num_orcamento' and cod_contas_a_receber = '$cod_contas_a_receber' and num_mensalidade = '$num_mensalidade' and categoria_conta = 'VENDA DE PRODUTOS' and banco = '$banco' and agencia = '$agencia' and contacorrente = '$contacorrente' and tipoconta = '$tipoconta'";
...
$comando = "update `$linha[1]`.`contas_a_receber` set `codigo` = '$cod_contas_a_receber',`quitacao`= 'Recebido',`bco_operacao`= '$banco',`agencia`= '$agencia',`contacorrente`= '$contacorrente',`tipoconta`= '$tipoconta' where `contas_a_receber`. `codigo` = '$cod_contas_a_receber' limit 1 ";
...
$sql3 = "SELECT * FROM contas_a_pagar where codigo_orcamento = '$codigo_orcamento' and num_mensalidade = '$num_mensalidade'";
...
$sql6 = "SELECT * FROM cx_saidas where num_fatura = '$num_fatura' and num_orcamento = '$num_orcamento' and cod_contas_a_pagar = '$cod_contas_a_pagar' and num_mensalidade = '$num_mensalidade' and categoria_conta = 'Despesas Recebimento Com Cartao'";
$res6 = mysql_query($sql6);

$lancamentos = mysql_num_rows($res6);

if($lancamentos>=1){

echo "Valor da despesa referente a fatura $num_fatura já registrado na saida do caixa!!!... Por essa razão não foi lançado novamente!<br> ";
...
$comando = "insert into cx_saidas(valor,dia,mes,ano,datacadastro,horacadastro,nfantasia,historico,categoria_conta,datecadastro,nome,cpf,operador,cel_operador,email_operador,estabelecimento,cidade_estabelecimento,tel_estabelecimento,email_estabelecimento,num_fatura,cod_contas_a_pagar,num_mensalidade,codigo_orcamento) 



values('$valor_a_pagar','$dia','$mes','$ano','$datacadastro','$horacadastro','$estabelecimento','Fatura $num_fatura, Orcamento $num_orcamento - Pagto de despesa com $modopagto cartao $cartao, parcela $num_mensalidade de $quantparc','$categoria_conta_a_pagar','$datecadastro','$nome_cliente','$cpf','$nome_op','$celular_op','$email_op','$estab_pertence_op','$cidade_estabelecimento_alterou','$tel_estabelecimento_alterou','$email_estabelecimento_alterou','$num_fatura','$cod_contas_a_pagar','$num_mensalidade','$codigo_orcamento')";
...
echo "<br> Despesa de cartao referente a fatura $num_fatura no valor de R$ $valor_a_pagar lançada na saida de caixa com sucesso!<br>";
