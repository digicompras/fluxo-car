...
$data_comunicado = date('d/m/Y');

$dia_comunicado = date('d');
$mes_comunicado = date('m');
$ano_comunicado = date('Y');
...
$sql = "SELECT * FROM estabelecimentos where cnpj = '$cnpj_concessionaria' limit 1";
$res = mysql_query($sql);
while($linha=mysql_fetch_row($res)) {


$valor = $linha[42];
$dias_contrato = $linha[43];
...
$dias = round(($d3/60/60/24)+$dias_contrato);
#converter o tempo em horas
$hrs = round(($d3/60/60));
#converter o tempo em minutos
$mins = round(($d3/60));
//PARA DESCOBRIR QUAL DATA SERÁ DAQUI A 5 DIAS
$data_vencimento_contrato = date('d/m/Y',mktime(0,0,0,date('m'),date('d')+$dias_contrato,date('Y')));


// a função explode é usada para separar uma string em
// uma matriz de strings usando um delimitador

$data_vencto = $data_vencimento_contrato;
$data_vencto2 = explode("/", $data_vencimento_contrato);

$dia_vencimento_contrato = $data_vencto2[0];
$mes_vencimento_contrato = $data_vencto2[1];
$ano_vencimento_contrato = $data_vencto2[2];
...
?>
<? } ?>
<div align="center"><strong>COMUNICADO DE VENDA DE VE&Iacute;CULO - <?  echo $dias; ?></strong></div>
...
<table width="100%" border="0" cellspacing="4">
    ...
    <tr>
      ...
      <td><strong><font color="#0000FF">
        ...
        <? if($dia_inicio_contrato==0){echo $dia_comunicado; } else {echo $dia_inicio_contrato; } ?>
        <input name="dia_inicio_contrato" type="hidden" id="dia_inicio_contrato" value="<? if($dia_inicio_contrato==0){echo $dia_comunicado; } else {echo $dia_inicio_contrato; } ?>">
/ <strong><font color="#0000FF">
<? if($mes_inicio_contrato==0){echo $mes_comunicado; } else {echo $mes_inicio_contrato; } ?>
<input name="mes_inicio_contrato" type="hidden" id="mes_inicio_contrato" value="<? if($mes_inicio_contrato==0){echo $mes_comunicado; } else {echo $mes_inicio_contrato; } ?>">
</font></strong>/<strong><font color="#0000FF">
<? if($ano_inicio_contrato==0){echo $ano_comunicado; } else {echo $ano_inicio_contrato; } ?>
<input name="ano_inicio_contrato" type="hidden" id="dia_inicio_contrato3" value="<? if($ano_inicio_contrato==0){echo $ano_comunicado; } else {echo $ano_inicio_contrato; } ?>">
</font></strong></font></strong></font></strong></td>
      <td>T&eacute;rmino Contrato </td>
      <td><strong><font color="#0000FF">
        <strong><font color="#0000FF">
        <? if($dia_termino_contrato==0){echo $dia_vencimento_contrato; } else {echo $dia_termino_contrato; } ?>
        <input name="dia_termino_contrato" type="hidden" id="dia_termino_contrato" value="<? if($dia_termino_contrato==0){echo $dia_vencimento_contrato; } else {echo $dia_termino_contrato; } ?>">
/ <strong><font color="#0000FF">
<? if($mes_termino_contrato==0){echo $mes_vencimento_contrato; } else {echo $mes_termino_contrato; } ?>
<input name="mes_termino_contrato" type="hidden" id="mes_termino_contrato" value="<? if($mes_termino_contrato==0){echo $mes_vencimento_contrato; } else {echo $mes_termino_contrato; } ?>">
</font></strong>/<strong><font color="#0000FF">
<? if($ano_termino_contrato==0){echo $ano_vencimento_contrato; } else {echo $ano_termino_contrato; } ?>
<input name="ano_termino_contrato" type="hidden" id="ano_inicio_contrato" value="<? if($ano_termino_contrato==0){echo $ano_vencimento_contrato; } else {echo $ano_termino_contrato; } ?>">
</font></strong></font></strong></font></strong></td>
    </tr>
    <tr> 
...
$senha = $_SESSION['senha'];
?>
        <input type="submit" name="Submit" value="Efetivar comunicado de venda"> 
