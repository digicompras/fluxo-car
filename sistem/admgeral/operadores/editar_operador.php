...
<table width="100%" border="0" cellspacing="4">
    ...
    <tr>
      ...
      <td><select name="bloqueio_parcial" id="bloqueio_parcial">
        <option selected><? echo $bloqueio_parcial; ?></option>
        <option>Não</option>
        <option>Sim</option>
      </select></td>
      <td>Bloqueio Compra de Divida</td>
      <td><select name="bloqueio_compra" id="bloqueio_compra">
...
Use o formato 00000-000</td>
    ...
    <tr>
      ...
      <td><textarea name="obs" rows="7" id="obs"><? echo $obs; ?></textarea></td>
      <td>Regiao</td>

      <td><select name="regiao" id="regiao">
        <option selected><? echo "$regiao"; ?></option>
        <?


    $sql = "select * from regioes order by codigo asc";
    $result = mysql_query($sql);
    while($x=mysql_fetch_array($result)){
  echo "<option>".$x['regiao']."</option>";
    }
?>
      </select></td>
