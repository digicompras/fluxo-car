...
$sql = "SELECT * FROM cx_abertura where operador = '$nome_operador' and dataabertura = '$datadeontem' order by dataabertura desc limit 1";
$res = mysql_query($sql);
while($linha=mysql_fetch_row($res)) {
$registro_de_abertura_de_ontem = mysql_num_rows($res);
	
$data_de_abertura_de_ontem = $linha[1];
...
$sql2 = "SELECT * FROM cx_fechamento where operador = '$nome_operador' and datafechamento = '$datadeontem' order by datafechamento desc limit 1";
$res2 = mysql_query($sql2);
while($linha=mysql_fetch_row($res2)) {
//$registro_de_fechamento = mysql_num_rows($res2);
	$registro_de_fechamento_ontem = mysql_num_rows($res2);
...
?></td>
	...
	<form action="caixa/verifica_caixa_periodico.php" method="post" name="form2" target="_blank" id="form14">
	  <div align="center">
	    <?
...
?>
	    <?
		if(($reg_mensagem==0) && ($registro_de_abertura>=1) && ($registro_de_fechamento>=1) && ($nome_operador=="Claudia Chaves Vieirra") ){
	    ...
	    <? if(($reg_mensagem==0) && ($registro_de_abertura>=1) && ($registro_de_fechamento>=1) && ($nome_operador=="Claudia Chaves Vieirra") ){ echo "<input class='class01' type='submit' name='Submit5' value='Visualizar Caixa'>"; } ?>
	    </div>
	  </form>
	<?
		echo "<br>";
		if(($reg_mensagem==0) && ($registro_de_fechamento_ontem==1) && ($registro_de_abertura==1) && ($registro_de_fechamento==1)){
		
		if($periodo=="noturno"){
		$sql = "SELECT * FROM cx_fechamento where operador = '$nome_operador' and datafechamento = '$data_de_ontem_obtida' order by datafechamento desc limit 1";
		}
		else{
...
$data_de_fechamento = $linha[1];
	$hora_de_fechamento = $linha[2];
	$valor_de_fechamento = $linha[3];
	$valordinheiro_de_fechamento = $linha[7];
	$valorcartao_de_fechamento = $linha[8];
	$valorsaidas_de_fechamento = $linha[9];
	$saldo_de_fechamento = $linha[10];
...
CARTOES: R$ $valorcartao_de_fechamento<br><br>
...
?>
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit30' value='Produtos'> $registro_de_fechamento";
		}
		}
		else{
...
if($periodo=="noturno"){
		...
		if(($datecadastro==$data_de_abertura) && ($registro_de_abertura>=1)){
		
		}
		else{
		echo'<input class="class01" type="submit" name="Submit2" value="Abertura de caixa">';
		
		if(($datecadastro>$data_de_abertura) && ($registro_de_abertura_de_ontem>=1) && ($registro_de_abertura<=0)){
...
?>
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit5' value='Mapa de Mesas'>";
		}
		}
		else{
...
?>
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit5' value='Movimento'>";
		}
		}
		else{
...
?>
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit5' value='CAIXA'>"; 
		}
		}
		else{
...
?>
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit5' value='Pesquisar'>"; 
		}
		}
		else{
...
?>
    ...
    <td colspan="4" style="text-align: center"><form action="index.php" method="post" name="form2" id="form10">
      <div align="center">
...
?>
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit5' value='FECHAMENTO DE CAIXA'>";
		}
		}
		else{
  ...
  <tr>
    ...
    <td colspan="4"><div align="center">
      <form action="caixa/verifica_caixa_periodico.php" method="post" name="form2" id="form9">
        <div align="center">
		<?
		...
		if(($reg_mensagem==0) && ($registro_de_abertura>=1) && ($registro_de_fechamento==0) && ($fechamentodecaixa=="fechar caixa") && ($hora_atual>="06:00:00")){
		

		echo "$data_de_abertura_de_ontem";
		echo "<input name='datefechamento' type='hidden' id='datefechamento' value='$data_de_abertura_de_ontem' />
		...
		if(($reg_mensagem==0) && ($registro_de_abertura>=1) && ($registro_de_fechamento==0) && ($fechamentodecaixa=="fechar caixa")){
		echo "<input name='datefechamento' type='hidden' id='datefechamento' value='$data_de_abertura' />
          <input name='solicitacao_gravar_fechamento_caixa' type='hidden' id='solicitacao_gravar_fechamento_caixa' value='gravarfechamentocaixa' />";
		  }
		  }
		...
		if($datecadastro>=$data_de_abertura){
		$sql = "SELECT * FROM cx_abertura where operador = '$nome_operador' and departamento = '$ultimo_departamento_trabalhado' and dataabertura = '$datadeontem' order by datecadastro desc limit 1";
$res = mysql_query($sql);
while($linha=mysql_fetch_row($res)) {
...
$sql = "SELECT * FROM cx_abertura where operador = '$nome_operador' and departamento = '$ultimo_departamento_trabalhado' order by datecadastro desc limit 1";
$res = mysql_query($sql);
while($linha=mysql_fetch_row($res)) {
...
if($periodo=="noturno"){
		...
		if(($datecadastro>=$data_de_abertura) && ($fechamentodecaixa=="fechar caixa") && ($hora_atual>="06:00:00")){
		 echo "Tem Certeza que deseja fechar o Caixa?<br>"; 
		 
		 
		 $sql = "SELECT * FROM faturamento_futuro where operador = '$nome_operador' and datefaturamento = '$data_do_ultimo_registro_de_abertura_de_caixa' and status_fatura = 'Aberto' order by datefaturamento desc";
$res = mysql_query($sql);
$registro_de_faturas_e_comandas_abertas_de_ontem = mysql_num_rows($res);
	

		if($registro_de_faturas_e_comandas_abertas_de_ontem>=1){
...
$num_fatura_aberta = $linha[1];
$comanda_utilizada_aberta = $linha[46];

echo "As faturas e suas respectivas comandas ainda se encontram em Aberto.... Favor finalizar todas para depois fechar o caixa!<br><br>
$num_fatura_aberta - $comanda_utilizada_aberta<br><br>
";
		...
		if(($fechamentodecaixa=="fechar caixa") && ($hora_atual<="05:59:59")){
		echo "Nao e hora de fechar o caixa ainda!!!... Seu expediente termina as 06:00:00<br>";
		
		//EMAIL DO ADMINISTRADOR QUE VAI RECEBER O PEDIDO
	$email_dest   =   "$email_empresa";
	$email_dest2   =   "ivancamposaraujo@gmail.com";
	$email_dest3   =   "nardocarvalhos@gmail.com";
	$mensagem_do_email = "Tentativa de Fechar o Caixa antes do horario";
		
		//PREPARA O PEDIDO
	$mens   =  "Tentativa de Fechar o Caixa antes do horario \n";
...
$subject = "Tentativa de Fechar o Caixa antes do horario";
$html = "
<html>
<body>
Olá $proprietarios! Tentativa de Fechar o Caixa antes do horario na <b>$nfantasia_empresa<b>!<br>


Data: $datecadastro<br>
Hora: $hora_atual<br>
...
//echo "Ocorreu um erro durante o envio do email.";
	  ...
	  if($fechamentodecaixa=="fechar caixa"){ echo "Tem Certeza que deseja fechar o Caixa?<br>"; 
	  } 
	  }
	  ?>
          <?
...
?>
		...
		echo "
		<input class='class02' name='dia_inicial' type='hidden' id='dia_inicial' size='10' value='$dia_de_ontem_obtido'>
		<input class='class02' name='mes_inicial' type='hidden' id='mes_inicial' size='10' value='$mes_de_ontem_obtido'>
        <input class='class02' name='ano_inicial' type='hidden' id='ano_inicial' size='10' value='$ano_de_ontem_obtido'>
        
        <input class='class02' name='dia_final' type='hidden' id='dia_final' size='10' value='$dia_de_ontem_obtido'>
        <input class='class02' name='mes_final' type='hidden' id='mes_final' size='10' value='$mes_de_ontem_obtido'>
        <input class='class02' name='ano_final' type='hidden' id='ano_final' size='10' value='$ano_de_ontem_obtido'>
        ";
		...
		if(($datecadastro>=$data_de_abertura) && ($fechamentodecaixa=="fechar caixa")){
		
		}
		else{
		echo "<input class='class01' type='submit' name='Submit5' value='Visualizar Caixa'>";
		}
		}
		else{
		
		  if(($reg_mensagem==0) && ($registro_de_abertura>=1) && ($registro_de_fechamento==0) && (empty($fechamentodecaixa)) ){ echo "<input class='class01' type='submit' name='Submit5' value='Visualizar Caixa'>"; 
		...
		echo "$data_do_ultimo_registro_de_abertura_de_caixa_para_fechamento";
		echo "<input class='class02' name='solicitacao_gravar_fechamento_caixa' type='hidden' id='solicitacao_gravar_fechamento_caixa' size='10' value='gravarfechamentocaixa'>
		<input class='class02' name='datefechamento' type='hidden' id='datefechamento' size='10' value='$datadeontem'>
		";
		
		
		echo "<input class='class01' type='submit' name='Submit5' value='Sim'>";
		}
		}
		else{
...
?>
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td colspan="4">
      <form action="index.php" method="post" name="form2" id="form15">
        <div align="center">
          <?
...
?>
		...
		if(($datecadastro>=$data_de_abertura) && ($fechamentodecaixa=="fechar caixa") && ($hora_atual>="06:00:00")){
		echo "<input class='class01' type='submit' name='Submit5' value='Nao'>";
		}
		}
		else{
	...
	if($periodo=="noturno"){
		...
		if($datecadastro>=$data_de_abertura){
		echo "Impressão de Comandas";
		}
		}
		else{
		
	if(($reg_mensagem==0) && ($registro_de_abertura>=1) && ($registro_de_fechamento==0) ){ echo "Impressão de Comandas"; 
	} 
	}
	?></td>
  </tr>
...
if($periodo=="noturno"){
		...
		if($datecadastro>=$data_de_abertura){
		echo "<input class='class01' type='submit' name='Submit5' value='Imprimir'>"; 
		}
		}
		else{
