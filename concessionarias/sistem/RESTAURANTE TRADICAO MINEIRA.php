...
if(($registro_abertura_de_caixa==0) && ($registro_fechamento_de_caixa_de_ontem==1) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA")){
...
?>
        <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA")){ echo "<input class='class01' type='submit' name='Submit5' value='Mapa de Mesas'>"; } ?>
      </div>
    </form></td>
...
?>
          <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE")){ echo "<input class='class01' type='submit' name='Submit5' value='Movimento'>"; } ?>
        </div>
...
?>
        <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE")){ echo "<input class='class01' type='submit' name='Submit5' value='CAIXA'>"; } ?>
      </div>
    </form></td>
...
?>
        ...
        <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE")){ echo "<input class='class01' type='submit' name='Submit5' value='Pesquisar'>"; } ?>
      </div>
    </form></td>
...
$total_liquido_faturas_fechadas = bcadd($linha['total_liquido_faturas_fechadas'],0,2);
  ...
  <tr>
    ...
    <td><form action="index.php" method="post" name="form2" id="form6">
      <div align="center">
        <?
		if(($registro_abertura_de_caixa==0) && ($solicitacao=="abrircaixa") && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA")){
...
?>
        ...
        <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE")){ echo "<input class='class01' type='submit' name='Submit5' value='Visualizar Caixa'>"; } ?>
        </div>
    </form>     </td>
    <td>&nbsp;</td>
    <td><div align="center">
...
?>
          <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA")){ echo "<input class='class01' type='submit' name='Submit5' value='FECHAMENTO DE CAIXA'>"; } ?>
          <? if(($registro_abertura_de_caixa_de_ontem==1) && ($registro_fechamento_de_caixa_de_ontem==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA")){ echo "<input class='class01' type='submit' name='Submit5' value='FECHAMENTO DE CAIXA'>"; } ?>
          <?
...
?>
          <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") && ($fechamentodecaixa=="fechar caixa") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") && ($fechamentodecaixa=="fechar caixa") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE") && ($fechamentodecaixa=="fechar caixa")){ echo "<input class='class01' type='submit' name='Submit5' value='Nao'>"; } ?>
        </div>
      </form>
    </div></td>
...
?>
        <? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") && ($fechamentodecaixa=="fechar caixa") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") && ($fechamentodecaixa=="fechar caixa") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE") && ($fechamentodecaixa=="fechar caixa")){ echo "<input class='class01' type='submit' name='Submit5' value='Sim'>"; } ?>
      </div>
    </form></td>
    <td>&nbsp;</td>
    <td><div align="center">
...
<? if(($registro_abertura_de_caixa==1) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($registro_fechamento_de_caixa_de_ontem==0) && ($registro_fechamento_de_caixa==0) && ($reg_mensagem==0) && ($ultimo_departamento_trabalhado=="RESTAURANTE TRADICAO MINEIRA") or ($reg_mensagem==0) && ($ultimo_departamento_trabalhado<>"") && ($funcao=="GERENTE")){ echo "<input class='class01' type='submit' name='Submit5' value='Imprimir'>"; } ?>
        </div>
      </form>
    </div></td>
  </tr>
