...
$estab_pertence = $linha[44];
 ...
 ?>
    ...
    <tr>
      <td width="19%">&nbsp;</td>
      <td><strong><font color="#0000FF" size="4">Entrada no Estoque?</font></strong></td>
    </tr>
    <tr>
      <td><form action="../menu.php" method="post" name="form1">
...
?>
    ...
    $sql = "select * from fornecedores where nfantasia = '$estab_pertence' order by nfantasia asc";
...
?>
        ...
        <input class='class01' type="submit" name="Submit2" value="Entrada">
        <span class="style1">
        <?
$usuario = $_SESSION['usuario'];
$senha = $_SESSION['senha'];
...
$sql = "SELECT * FROM estoque_pecas where nome_produto = '$nome_produto' limit 1";
$res = mysql_query($sql);
while($linha=mysql_fetch_row($res)) {
...
$margem_folga = $linha[32];
  ...
  ?>
    <tr>
      ...
      <td width="21%"><input class='class02' name="nome_produto" type="text" id="nome_produto" value="<? echo $nome_produto; ?>" size="40"></td>
<td width="53%"><input class='class01' type="submit" name="Submit3" value="Buscar">
          <span class="style1">
          <?
$usuario = $_SESSION['usuario'];
$senha = $_SESSION['senha'];
...
$comando = "update `$linha[1]`.`estoque_pecas` set `codigo` = '$codigoproduto',`quant_estoque` = '$quantidade',`preco_compra` = '$preco_compra',`preco` = '$preco',`oferta` = '$oferta',`preco_oferta` = '$preco_oferta' where `estoque_pecas`. `codigo` = '$codigoproduto' limit 1 ";
}
mysql_query($comando,$conexao);
...
$sql = "select * from estoque_pecas where nome_produto like '$nome_produto%' order by nome_produto asc";
$res = mysql_query($sql);
while($linha=mysql_fetch_row($res)) {
...
<form name="form1" method="post" action="menu.php">
  <tr class="style3">
    ...
    <td width="5%" class="style2"><div align="center">
      <input class='class02' name="nf" type="text" id="nf" size="4">
    </div></td>
    <td width="14%" class="style2"><div align="center">
      <select class='class02' name="dia_nf" id="dia_nf">
        <option>01</option>
        <option>02</option>
        <option>03</option>
        <option>04</option>
...
<?  echo $quant_estoque; ?>
<input class='class02' name="quantidade" type="text" id="quantidade" size="4">
<select class='class02' name="comando" id="comando">
  <option>E</option>
  <option selected>A</option>
</select>
    </div></td>
    <td width="8%"><div align="center">
<input class='class02' name="preco_compra_atual" type="text" id="preco_compra2" value="<? echo $preco_compra; ?>" size="4">
</div></td>
    <td width="7%"><div align="center">
      <?  echo $preco; ?>
      <input name="preco" type="hidden" id="preco" value="<? echo $preco; ?>">
    </div></td>
...
<select class='class02' name="oferta" id="oferta">
  ...
  <option>N&atilde;o</option>
    ...
    <td width="7%"><div align="center">
      <input class='class02' name="preco_oferta" type="text" id="preco_oferta" value="<? echo $preco_oferta; ?>" size="4">
    </div></td>
